	...
	<div class="page-header-content header-elements-md-inline">
		...
		<div class="header-elements d-none py-0 mb-3 mb-md-0">
			<div class="breadcrumb">
				...
				<span class="breadcrumb-item active"> @lang('bill_types.show_all_bill_types') </span>
			</div>
		</div>
	</div>
</div>
	...
	<div class="col-md-12">
		<div class="header-elements  ">
			<div class="float-right">
				<a href="{{ route('bill_types.create') }}" class="btn btn-primary ml-1"> <i class="icon-plus3"></i>  @lang('bill_types.add_new_type') </a>
			</div>
		</div>
	</div>
	<hr>
			...
			<table class="table  table-xs table-bordered">
				<thead class="bg-dark">
					<tr>
						<th></th>
						<th> # </th>
						<th> @lang('bill_types.name_en') </th>
						<th> @lang('bill_types.name_ar') </th>
						<th> @lang('bill_types.added_by') </th>
						<th> @lang('bill_types.created_at') </th>
					</tr>
				</thead>
				<tbody>
					@php
					...
					<tr>
						...
						<td>  {{ $type->type_ar }} </td>
						<td> {{ $type->type_en }} </td>
						<td> <a href="{{ route('admins.show'  , ['admin' => $type->admin_id ] ) }}"> {{ optional($type->admin)->name }} </a> </td>
					...
					<tr class="collapse " id="collapse-icon{{ $type->id }}" >
						<td colspan="100%" >
							...
							<div class="float-left">
								@lang('bill_types.settings') :
								<a target="_blank" href="{{ route('bill_types.edit' , ['bill_type' => $type->id ] ) }}" class="btn alpha-warning border-warning text-warning-800 btn-icon ml-2">
	...
	function confirm_deletion() {
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url: '{{ url('Board/ajax/delete_bill_type') }}',
					type: 'POST',
					dataType: 'json',

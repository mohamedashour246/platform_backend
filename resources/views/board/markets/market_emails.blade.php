	...
	<div class="col-md-10">
		...
		<div class="card">
			<div class="card-header bg-dark header-elements-inline">
				<h5 class="card-title"> @lang('markets.market_emails') </h5>
				<div class="header-elements">
			...
			<div class="card-body">
				<table class="table  table-xs border-top-0 my-2">
					...
					<thead>
						<tr>
							<th> # </th>
							<th> @lang('markets.email') </th>
							<th> @lang('markets.notes') </th>
					...
					<tbody>
						...
						$i = 1;
						@endphp
						@foreach ($market->emails as $email)
						<tr>
							<td> {{ $i++ }} </td>
							<td> {{ $email->email }} </td>
							<td> {{ $email->notes }} </td>
							<td> <a href="{{ route('admins.show'  , ['admin' => $email->admin_id ] ) }}">  {{ optional($email->admin)->username }} </a> </td>
							<td>
								<a href="#" class="btn btn-warning btn-sm  ml-1"> <i class="icon-pencil5"></i> @lang('markets.edit') </a>
								<a href="#" class="btn btn-danger  btn-sm ml-1"> <i class="icon-trash"></i> @lang('markets.delete') </a>
							</td>
						</tr>
						@endforeach
					</tbody>

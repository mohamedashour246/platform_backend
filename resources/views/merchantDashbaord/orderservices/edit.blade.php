...
@section('title')
    @lang('merchantDashbaord.edit_order')
@endsection
    ...
    <div class="page-header">
        <div class="page-header-content header-elements-md-inline">
            <div class="page-title d-flex">
                <h4><i class="icon-arrow-right6 mr-2"></i> @lang('merchantDashbaord.edit_order') </h4>
        ...
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-header bg-dark header-elements-inline">
                    <h5 class="card-title"> @lang('merchantDashbaord.edit_order') </h5>
                ...
                {!! Form::model($orderservice, ['route' => ['orderservices.update', $orderservice->id], 'method' => 'patch', 'files' => true]) !!}
                    @method('patch')
                    <div class="card-body">
                        @csrf
                        <fieldset>
                            <legend class="font-weight-bold"> <span class="text-primary"> @lang('merchantDashbaord.edit_order') </span> </legend>
                           <div class="row">
                               <!-- Name Ar Field -->
                               <div class="form-group col-sm-12">
                                 {!! Form::label('type', __('merchantDashbaord.order_type')) !!}
                                 <select class="form-control" name="type">
                                      @if($orderservice->type == 'طلب عادى')
                                          <option selected> طلب عادى </option>
                                          <option> طلب مستقبلى </option>
                                      @endif
                                      @if($orderservice->type == 'طلب مستقبلى')
                                          <option value="{{$orderservice->id}}" selected> طلب مستقبلى </option>
                                          <option>  طلب عادى  </option>
                                      @endif
                                 </select>
                                 @error('type')
                               ...
                               <div class="form-group col-sm-12">
                                 ...
                                 <select class="form-control" name="merchant_id">
                                   <option value="{{$orderservice->merchant_id}}"> {{$orderservice->merchant->username}}  </option>
                                      @foreach($merchants as $merchant)
                                          @if($merchant->id != $orderservice->merchant_id)
                               ...
                               <div class="form-group col-sm-6">
                                 ...
                                 <select class="form-control" name="payment_type">
                                      @if($orderservice->payment_type == 'كاش')
                                        <option selected> كاش  </option>
                                        <option> كى نت </option>
                                      @endif
                                      @if($orderservice->payment_type == 'كى نت')
                                        <option selected> كى نت  </option>
                                        <option> كاش </option>
                                      @endif
                                 </select>
                                 @error('payment_type')
                               ...
                               <div class="col-lg-12 col-form-label">
                                   ...
                                   <div class="col-lg-9">
                                      <div class="items-container col-12 tBody">
                                        <div class="row">
                                           <input type="number" name="count" value="{{$productorder->count}}" placeholder="العدد" class="col-2 form-control m-1">
                                           @error('count')
                                           <label class="text-danger font-weight-bold" > {{ $message }} </label>
                                           @enderror
                                           <input type="text" name="item_ar" value="{{$productorder->name_ar}}" placeholder="الاسم بالعربى"  class="col-4 form-control m-1">
                                           @error('item_ar')
                                           <label class="text-danger font-weight-bold" > {{ $message }} </label>
                                           @enderror
                                           <input type="text" name="item_en" value="{{$productorder->name_en}}" placeholder="الاسم بالانجليزى" class="col-4 form-control m-1">
                                           @error('item_en')
                                           <label class="text-danger font-weight-bold" > {{ $message }} </label>
                                           @enderror
                                           <div class="ml-3">
                               ...
                               <div class="form-group col-sm-6">
                                 ...
                                 <select class="form-control" name="user_id">
                                      <option value="{{$orderservice->user_id}}"> {{$orderservice->user->name}}  </option>
                               ...
                               <div class="form-group col-sm-12">
                                 ...
                                 <select class="form-control" name="driver_id">
                                     <option value="{{$orderservice->driver_id}}"> {{$orderservice->driver->name}}  </option>
                                      @foreach($drivers as $driver)
                                          @if($driver->id != $orderservice->driver_id)
